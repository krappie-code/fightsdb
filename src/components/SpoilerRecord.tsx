...
export function SpoilerRecord({ wins, losses, draws, className = '', size = 'sm' }: SpoilerRecordProps) {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 ${className}`}>
      {visible ? (
        <>
          <span className={`font-mono text-zinc-${size === 'lg' ? '200' : '300'} ${textSize}`}>
            {wins}-{losses}-{draws}
          </span>
          <button
        ...
        <>
          <span className={`font-mono text-zinc-600 ${textSize}`}>
            ●●-●●-●●
          </span>
          <button
